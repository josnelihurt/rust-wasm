...
pub fn link_prg(
    ...
    gl.attach_shader(&prg, &vert_shader);
    gl.attach_shader(&prg, &frag_shader);
    gl.link_program(&prg);

    if gl.get_program_parameter(&prg, WebGlRenderingContext::LINK_STATUS)
            .as_bool()
            .unwrap_or(false)
    {
        Ok(prg)
    } else {
        Err(gl.get_program_info_log(&prg)
...
fn compile_shader(
    ...
    gl.shader_source(&shader, src_code);
